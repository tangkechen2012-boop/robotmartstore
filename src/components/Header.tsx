      ...
      <header className={`sticky top-0 z-50 w-full transition-all duration-300 ${scrolled ? "py-1" : "py-2"}`}>
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-4">
          ...
          <Link to="/" className="flex-shrink-0 flex items-center">
            <img alt="RobotMart" className="h-12 w-auto object-contain" src="/lovable-uploads/f038be6c-9698-4016-9a23-89001cae8e9f.png" />
          </Link>
              ...
              <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
                ...
                <SheetContent side="left" className="w-80 overflow-y-auto rounded-r-2xl">
                  ...
                  <nav className="space-y-1">
                    ...
                    <Link to="/contact" className="block py-3 px-4 rounded-xl text-sm font-semibold bg-primary text-primary-foreground text-center mt-4" onClick={() => setMobileOpen(false)}>
                      Contact Us
                    </Link>
                  </nav>
                </SheetContent>
                ...
                <div
                  ...
                  <div
                    ...
                    <div key={section.title}>
                        ...
                        <li key={sub.label}>
                                  <Link to={sub.link} className="text-sm text-muted-foreground hover:text-foreground transition-colors duration-250 block py-1">
                                    {sub.label}
                                  </Link>
                                </li>
                        )}
...
function MobileNavItem({ item, onClose }: {item: NavItemType;onClose: () => void;}) {
  ...
  if (!item.sections) {
    return (
      <Link to={item.link} className="block py-2.5 px-4 rounded-xl text-sm font-medium hover:bg-secondary transition-colors" onClick={onClose}>
        {item.label}
      </Link>);
  ...
  return (
    <div>
      <button className="w-full flex items-center justify-between py-2.5 px-4 text-sm font-medium hover:bg-secondary rounded-xl transition-colors" onClick={() => setOpen(!open)}>
        {item.label}
      ...
      <div className="pl-4 space-y-2 pb-2">
        ...
        <div key={section.title}>
              ...
              {section.items.map((sub) =>
          <Link key={sub.label} to={sub.link} className="block text-sm text-muted-foreground hover:text-foreground px-4 py-1.5" onClick={onClose}>
                  {sub.label}
                </Link>
          )}
            </div>
